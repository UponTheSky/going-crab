...
There are many ways to do so, depending on how you deploy your server. Using Kubernetes or cloud vendor services like AWS EC2, you may do configuration using bunch of `.yaml` files or tools like [Terraform](https://developer.hashicorp.com/terraform). However, in this chapter, we will build our own way of injecting such config information into the server application from scratch. It is a rough practice, but will teach us how an application is configured in a secure way. 

But first, let's talk about environment variables.
